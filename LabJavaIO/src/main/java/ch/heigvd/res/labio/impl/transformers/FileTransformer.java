...
public abstract class FileTransformer implements IFileVisitor {
  ...
  public void visit(File file) {
    ...
    try {
      Reader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), "UTF-8")); // Bufferize the reader
      Writer writer = new OutputStreamWriter(new FileOutputStream(file.getPath()+ ".out"), "UTF-8"); // the bug fix by teacher
      writer = new BufferedWriter(decorateWithFilters(writer)); // Bufferize the writer after decorating
